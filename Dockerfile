...
WORKDIR /app
# Manifests before the sources, so the install layer keys only on the lock file
# and survives a source-only change.
COPY package*.json .npmrc ./
RUN npm ci
COPY . .
RUN npm run build && npm run build-storybook -- -o dist/storybook
