...
const LineProfile = () => {
  ...
  useEffect(() => {
    ...
    setRouteKey(`${route.routeMkt}-${route.routeDirection}-${route.routeAlternative}`)
  }, [route?.id])
  ...
  const handleTimestampChange = (time: dayjs.Dayjs | null) => {
    ...
    getRoutesAsync(
      ...
      .then((routes) => {
        const newRoute = routes?.find((r) => r.key === `${route.routeMkt}-${route.routeDirection}-${route.routeAlternative}`)
