...
describe('deviationSpans', () => {
  ...
  // A double trip runs one departure with two vehicles, which can miss the schedule in
  // opposite directions. The two blocks meet at the scheduled instant, so they never overlap.
  it('splits a departure that ran both early and late at the scheduled instant', () => {
  ...
  it('stretches one block to the worst vehicle rather than one block per vehicle', () => {
    expect(deviationSpans(band({ plannedTops: [100], actualTops: [130, 400, 210] }))).toEqual([
      { top: instantY(100), bottom: instantY(400), deviation: 'late' },
    ])
  })
