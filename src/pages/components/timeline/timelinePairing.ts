...
/**
 * One span per actual record: a departure can be run by more than one vehicle, and each
 * has its own deviation from the schedule — one may be early while another is late.
 * Measuring only the nearest would leave the rest of a double trip uncoloured.
 *
 * Measured centre to centre, so a span's height is exactly the delay: an on-time ride
 * spans nothing, and the bounding rules land on its edges rather than inside them.
 */
export function deviationSpans(band: TimelineBand): DeviationSpan[] {
  if (!band.plannedTops.length || !band.actualTops.length) return []
  const planned = Math.min(...band.plannedTops)

  return [...band.actualTops]
    .sort((a, b) => a - b)
    .map((actual) => ({
      top: instantY(Math.min(planned, actual)),
      bottom: instantY(Math.max(planned, actual)),
      // y grows with time, so a lower actual dot is a later arrival
      deviation: actual > planned ? 'late' : actual < planned ? 'early' : ('on-time' as const),
    }))
}
