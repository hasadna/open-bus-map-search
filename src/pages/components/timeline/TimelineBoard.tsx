 ...
 *
 * A departure run by several vehicles gets one lane each, side by side rather than stacked —
 * overlapping translucent fills would darken into a severity nobody claimed.
 */
const Band = styled.div<{
  ...
  position: absolute;
  left: ${({ $lane, $lanes }) => (100 * $lane) / $lanes}%;
  width: ${({ $lanes }) => 100 / $lanes}%;
  top: ${({ $top }) => $top}px;
